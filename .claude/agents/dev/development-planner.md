...
- 수락 기준과 완료 조건 정의
- **🚫 구현 완료 후 테스트 없이 다음 단계 진행 금지 — 예외 없음**
- **테스트 도구**: 반드시 Playwright MCP 활용 (브라우저 기반 실제 동작 검증)
- **테스트 대상**: API 엔드포인트, 비즈니스 로직, 인증 플로우, 폼 검증, 페이지 렌더링 등 모든 기능 구현
- 테스트 실패 시 반드시 원인 파악 및 수정 후 재테스트 — 실패 상태로 진행 불가
...
**구현 후 테스트는 선택이 아닌 필수입니다.** ROADMAP을 생성할 때, 그리고 실제 구현 단계에서 아래 원칙을 반드시 적용하세요.

#### **테스트 적용 기준**

다음 중 하나라도 해당하면 Playwright MCP 테스트 필수:

| 구분 | 예시 |
|------|------|
| API 엔드포인트 구현 | REST API, Server Actions, Route Handlers |
| 인증/권한 로직 | 로그인, 세션, 미들웨어 보호 |
| 비즈니스 로직 | 데이터 파싱, 계산, 상태 변환 |
| 폼 처리 | 검증, 제출, 오류 표시 |
| 외부 API 연동 | Notion, 결제, 메일 등 |
| 페이지 렌더링 | 데이터 fetch 후 화면 표시 |

#### **테스트 실행 절차**

1. 구현 완료 → 개발 서버 실행 확인
2. Playwright MCP로 브라우저를 조작하여 기능 검증
3. 정상 케이스(Happy Path): 예상대로 동작하는지 확인
4. 오류 케이스: 잘못된 입력/상태에서 적절한 에러 처리 확인
5. 엣지 케이스: 빈 값, 경계값, 극단값 테스트
6. 테스트 결과를 작업 파일(`tasks/XXX.md`)에 기록
7. **모든 케이스 통과 후에만 다음 단계 진행**
...
5. **테스트 체크리스트 필수**: API·비즈니스 로직·인증·외부 연동이 포함된 Task는 반드시 Playwright MCP 시나리오를 포함한 `## 테스트 체크리스트` 섹션 작성
...
- [ ] API·비즈니스 로직·인증·외부 연동이 포함된 모든 Task에 `## 테스트 체크리스트` 섹션이 있는가?
- [ ] 각 테스트 체크리스트에 Playwright MCP 시나리오가 구체적으로 작성되었는가?
- [ ] 정상 케이스(Happy Path)와 오류 케이스가 모두 포함되었는가?
- [ ] 에러 핸들링 및 엣지 케이스 테스트 시나리오가 정의되었는가?
- [ ] 개발 워크플로우에 "테스트 통과 후 다음 단계 진행" 원칙이 명시되었는가?
- [ ] Phase 3 이후 핵심 기능 구현 Task마다 통합 테스트가 포함되었는가?
- [ ] 모든 사용자 플로우(Happy Path 전체)에 대한 E2E 시나리오가 정의되었는가?
